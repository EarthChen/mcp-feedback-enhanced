...
- 🔧 **技能未注入 Agent**：WebSocket 提交反馈时补传 `skills` 字段，修复 `/skillname` 仅作为纯文本转发、未注入 `SKILL.md` 正文的问题。
- 🔧 **提交后高亮残留**：提交成功后清空输入框、草稿与高亮镜像层，修复 `/skill` 彩色底色在已提交状态下仍残留的问题。

## [v2.12.0] - 2026-08-24 - 心跳保活修复与技能交互增强
...
## [v2.11.1] - 2026-08-21 - 会话/标签页一对一对应修复

### 🐛 错误修复

- 🔧 **会话↔标签页对应**：多会话反馈现在会刷新「正确」的浏览器标签页。待发送的会话更新通知改为「按会话」追踪（不再是多会话并发会互相覆盖的单一全局标记），并建立活跃标签页注册表，使单一会话即使在 `mcp_session_id` 跨调用变动时，也能每次复用同一个标签页。已加入回归测试。

## [v2.11.0] - 2026-08-21 - Agent 技能支持

### ✨ 新功能

- 🤖 **Agent 技能触发**：现在通过将 `SKILL.md` 内容注入反馈来触发 Agent 技能，支持技能感知的交互。

## [v2.10.1] - 2026-08-21 - 反馈路由与客户端提交修复

### 🐛 问题修复

- 🔧 **WebSocket 会话路由**：入站消息改为解析当前真正拥有连接的会话，修复「持久化标签页在新 MCP 调用转移连接后，首次反馈无效、刷新才生效」的问题（新增回归测试）。
- 🔧 **待发反馈刷新**：客户端在连接就绪（connection_established 到达）时刷新待发反馈，修复连接就绪窗口内提交的反馈静默丢失；移除未使用的 WebSocketManager.pendingSubmission。
