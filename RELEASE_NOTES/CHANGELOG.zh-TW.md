...
## [v2.12.2] - 2026-08-24 - 技能注入可靠性增強

### 🐛 錯誤修復

- 🔧 **技能解析未掛載**：修復 `skill-autocomplete.js` 誤用 `window.FeedbackApp`（應為 `MCPFeedback.FeedbackApp`），導致提交時 `skills` 始終為空、`SKILL.md` 未注入的問題。
- 🔧 **隊列首條遺失 skills**：批量反饋模式下首條訊息現在也攜帶 `skills` 與 `project_directory`，避免技能注入被跳過。

### ✨ 新功能

- 🛡️ **服務端技能解析兜底**：當前端未附帶 `skills` 時，服務端從反饋正文解析 `/skillname` 並注入（仍經路徑白名單校驗）。
- 📝 **同行多 skill 解析**：同一行多個 `/skillname` 時，參數在下一個 skill 引用前截斷，例如 `/tdd 寫測試 /code-review 審查`。

## [v2.12.1] - 2026-08-24 - 技能注入與提交後高亮修復

### 🐛 錯誤修復

- 🔧 **技能未注入 Agent**：WebSocket 提交反饋時補傳 `skills` 欄位，修復 `/skillname` 僅作為純文字轉發、未注入 `SKILL.md` 正文的問題。
- 🔧 **提交後高亮殘留**：提交成功後清空輸入框、草稿與高亮鏡像層，修復 `/skill` 彩色底色在已提交狀態下仍殘留的問題。

## [v2.12.0] - 2026-08-24 - 心跳保活修復與技能互動增強

### 🐛 錯誤修復

- 🔧 **Cursor 长调用断连**：`interactive_feedback` 等待期间的心跳改为双通道——progress notification 之外，同时发送不依赖 progressToken 的 logging notification，修复客户端未提供 progressToken 时 stdio 完全静默、Cursor 按空闲超时掐断调用（约 5 分钟）的问题；心跳日志如实记录实际发送状态。
- 🔧 **技能自动补全失效**：事件监听改为 document 级委托，反馈输入框被 DOM 重建后无需重绑，↑↓/Enter/Tab/鼠标均可选中；升级静态资源版本号避免命中旧缓存。

### ✨ 新功能

- 🎨 **技能高亮**：输入框中所有已插入的 `/skill` 片段以彩色底色持续标记（textarea + 镜像层实现，滚动同步）。
- 💾 **草稿保护**：反馈文本实时存 localStorage（按会话键控），断连/刷新后自动恢复。

### 🔒 安全

- 🛡️ **技能注入路径校验**：仅接受服务端扫描清单内的 `SKILL.md`（按真实路径比对，兼容软链接约定），防止客户端提交任意文件路径造成注入；单文件超过 32KB 截断并标注。


## [v2.11.1] - 2026-08-21 - 會話/標籤頁一對一對應修復

### 🐛 錯誤修復

- 🔧 **會話↔標籤頁對應**：多會話回饋現在會重新整理「正確」的瀏覽器標籤頁。待發送的會話更新通知改為「按會話」追蹤（不再是多會話並發會互相覆蓋的單一全域標記），並建立活躍標籤頁註冊表，使單一會話即使在 `mcp_session_id` 跨呼叫變動時，也能每次復用同一個標籤頁。已加入回歸測試。

## [v2.11.0] - 2026-08-21 - Agent 技能支援

### ✨ 新功能

- 🤖 **Agent 技能觸發**：現在透過將 `SKILL.md` 內容注入回饋來觸發 Agent 技能，支援技能感知的互動。

## [v2.10.1] - 2026-08-21 - 回饋路由與客戶端提交修復

### 🐛 問題修復

- 🔧 **WebSocket 會話路由**：入站訊息改為解析當前真正擁有連線的會話，修復「持久化標籤頁在新 MCP 調用轉移連線後，首次回饋無效、刷新才生效」的問題（新增回歸測試）。
- 🔧 **待發回饋刷新**：客戶端在連線就緒（connection_established 到達）時刷新待發回饋，修復連線就緒窗口內提交的回饋靜默遺失；移除未使用的 WebSocketManager.pendingSubmission。
